...
LatentPPOLossInfo = collections.namedtuple(
    'LatentPPOLossInfo', ('ppo_loss', 'model_loss'))


@gin.configurable
class LatentPPOAgent(tf_agent.TFAgent):
  """TF agent with a sequential latent model and inner PPO agent."""
  ...
  def __init__(self,
    ...
    super(LatentPPOAgent, self).__init__(
        time_step_spec,
        action_spec,
        policy=policy,
  ...
  def _train(self, experience, weights=None):
    """Train both the inner ppo agent with the sequential latent model."""
    ...
    ppo_loss = self._inner_agent.train(latent_experience, weights).loss
    total_loss = ppo_loss + model_loss

    extra = LatentPPOLossInfo(ppo_loss=ppo_loss,
  ...
  def model_loss(self,
      ...
      with tf.name_scope('model_loss'):
        if self._model_batch_size is not None:
          ...
          if latent_posterior_samples_and_dists is not None:   #选择对应batch大小的后验采样样本和概率
